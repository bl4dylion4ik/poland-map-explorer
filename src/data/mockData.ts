...
function generateListings(): Listing[] {
  ...
  CITIES.forEach((city, cityIdx) => {
    ...
    for (let i = 0; i < listingCount; i++) {
      ...
      const monthOffset = Math.floor(seededRandom(seed + 3) * 12);
      const day = 1 + Math.floor(seededRandom(seed + 4) * 28);
      const date = `2024-${String(monthOffset + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
